...
// 初始 查的时候的数据从本地缓存中获取, 如果本地缓存没有，用默认数据
...
function renderHTML() {
  ...
  let str = ``
  // 拼接全选的数据
  str+=`<div class="top"><input class="toggleAll" type="checkbox" ${totalSelect === cartList.length?'checked':''} /> 全选</div> <ul>`
  // 拼接列表数据
  cartList.forEach((item)=>{
    str+=`
    <li>
        <div class="status">
          <input type="checkbox" ${item.status?'checked':''} class='checkbox' data-id='${item.id}' />
        </div>
        <div class="show">
          <img
            src="${item.pic}"
  ...
  str+=`</ul><div class="bottom">
          ...
          <div class="btns">
            ...
            <button class="buy" ${!totalSelect?'disabled':''} data-price='${totalPrice}' >去结算</button>
            <button class="removeComplete"  ${!totalSelect?'disabled':''} >删除所有已选中</button>
          </div>
          <div class="totalPrice">总价格 : ￥ <span>${totalPrice.toFixed(2)}</span></div>
        </div>`
  // 渲染
